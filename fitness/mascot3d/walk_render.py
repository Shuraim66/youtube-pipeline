"""
Apply the yellow + faceless dome + cyan visor to the RIGGED Meshy mesh, keeping the
armature/weights. Head verts (weighted to 'Head' bone) are reshaped into the dome;
the visor is welded into the mesh and weighted to 'Head' so it moves with the rig.
Saves mascot_rigged_yellow.blend (the rigged master) + a bind-pose render.
"""
import bpy, math
from mathutils import Vector
BASE="/home/alpha/products/youtube-pipeline/fitness/mascot3d"; GLB=f"{BASE}/rig/Meshy_AI_Iron_Titan_biped/Meshy_AI_Iron_Titan_biped_Animation_Walking_withSkin.glb"; R=f"{BASE}/renders"
...
# hue-shift skin -> yellow (+AO for muscle), matte
...
# world bounds (mesh may carry an import transform)
...
# reshape head into a dome (per-vertex, world space -> preserves weights)
...
mesh.data.update()
# flat yellow on the dome faces (hide baked face texture)
...
# visor bands (world coords), join into the mesh, weight the new verts to 'Head'
...
# render bind pose to confirm assembly
...
sc.render.resolution_x=620; sc.render.resolution_y=840; sc.frame_set(16); sc.render.filepath=f"{R}/ym_walk.png"
bpy.ops.render.render(write_still=True)
print("RIG YELLOW DONE")
