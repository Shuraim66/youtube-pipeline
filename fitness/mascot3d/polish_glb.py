...
# ---- body materials: skin / shoes / hair / shorts (painted, with depth gate) ----
skin=cel("Skin",(0.90,0.60,0.39),(0.50,0.29,0.18),ao_dist=0.05,ao_min=0.36)
shoes=cel("Shoes",(0.90,0.45,0.12),(0.5,0.24,0.06),ao_min=0.6)
hair=cel("Hair",(0.20,0.13,0.09),(0.10,0.06,0.04),ao_min=0.7)
shorts=cel("Shorts",(0.20,0.24,0.28),(0.10,0.12,0.15),ao_min=0.7)
body.data.materials.clear()
for m in (skin,shoes,hair,shorts): body.data.materials.append(m)
def is_shorts(c):
    dx=abs(c.x-cx); f=frac(c.z); fwd=c.y-cy       # fwd<0 = toward camera
    if dx>=xgate:                       return False   # arms hang beyond
    if fwd < -0.07*Hh and dx>0.05*Hh:   return False   # off-center forward = fist (keep centered crotch)
    if 0.45<f<0.575:                 return True   # waistband + hips
    if 0.30<f<=0.45 and dx>0.05*Hh:  return True   # legs down outer thighs
    return False
for poly in body.data.polygons:
    c=mw@poly.center; f=frac(c.z)
    if f<0.075:       poly.material_index=1
    elif f>0.945:     poly.material_index=2
...
emp.location=(cx,cy,cz)
body.parent=emp; body.matrix_parent_inverse=emp.matrix_world.inverted()

w=bpy.data.worlds.new("W"); bpy.context.scene.world=w; w.use_nodes=True
w.node_tree.nodes.get("Background").inputs[1].default_value=0.0
cd=bpy.data.cameras.new("C"); cd.type='ORTHO'; cd.ortho_scale=Hh*1.12
...
ls.select_crease=False               # muscle comes from AO; crease lines just speckle
...
for i,yaw in enumerate([0,25]):
    ...
    sc.render.filepath=f"{R}/polish_{i}.png"; bpy.ops.render.render(write_still=True); print("yaw",yaw)
print("DONE")
