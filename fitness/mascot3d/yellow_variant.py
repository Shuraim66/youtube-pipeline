"""
Apply the locked yellow + faceless cyan-visor dome treatment to a physique variant GLB.
  blender -b -P yellow_variant.py -- <name>     (reads mascot_<name>.glb -> mascot_<name>_yellow.glb + renders/var_<name>.png)
Same recipe as yellow_meshy.py (hue-shift skin->yellow, in-place head dome, cyan visor),
but parameterized per variant and it EXPORTS the finished GLB so each build is reusable.
"""
import bpy, bmesh, math, sys
name=sys.argv[sys.argv.index("--")+1] if "--" in sys.argv else "bulk"
BASE="/home/alpha/products/youtube-pipeline/fitness/mascot3d"; GLB=f"{BASE}/mascot_{name}.glb"; R=f"{BASE}/renders"
HUE=0.55; SAT=1.42; VAL=0.82
DOME=(0.82,0.50,0.015)
def _set(b,n,v):
...
body=bpy.context.active_object; body.name="VAR"; bpy.ops.object.shade_smooth()
...
# join visor into body and export the finished variant
bpy.ops.object.select_all(action='DESELECT')
for o in [o for o in bpy.data.objects if o.type=='MESH']: o.select_set(True)
bpy.context.view_layer.objects.active=body; bpy.ops.object.join(); body.name=f"Mascot_{name}"
bpy.ops.export_scene.gltf(filepath=f"{BASE}/mascot_{name}_yellow.glb", use_selection=True, export_format='GLB')

# render front check
piv=bpy.data.objects.new("Pivot",None); bpy.context.collection.objects.link(piv); piv.location=(cx,cy,cz)
body.parent=piv; body.matrix_parent_inverse=piv.matrix_world.inverted()
...
sc.render.engine='CYCLES'; sc.cycles.device='CPU'; sc.cycles.samples=130; sc.cycles.use_denoising=True
sc.render.resolution_x=760; sc.render.resolution_y=1040; sc.render.filepath=f"{R}/var_{name}.png"
