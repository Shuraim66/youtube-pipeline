...
GLB="/home/alpha/products/youtube-pipeline/fitness/mascot3d/mascot_shape.glb"
...
bpy.ops.object.shade_smooth()   # smooth normals -> cleaner cel bands
mw=body.matrix_world
vs=[mw@v.co for v in body.data.vertices]
zs=[p.z for p in vs]; xs=[p.x for p in vs]
zmin,zmax=min(zs),max(zs); Hh=zmax-zmin
cx=sum(xs)/len(xs)
def frac(z): return (z-zmin)/Hh
# materials (brand: charcoal shorts, orange accent, warm skin, dark hair)
skin=cel("Skin",(0.90,0.60,0.39),(0.50,0.29,0.18),ao_dist=0.05,ao_min=0.36)
shorts=cel("Shorts",(0.20,0.24,0.28),(0.10,0.12,0.15),ao_on=True,ao_min=0.65)  # charcoal
shoes=cel("Shoes",(0.90,0.45,0.12),(0.5,0.24,0.06),ao_on=True,ao_min=0.6)       # orange shoes
hair=cel("Hair",(0.20,0.13,0.09),(0.10,0.06,0.04),ao_on=True,ao_min=0.7)        # dark brown
body.data.materials.clear()
for m in (skin,shorts,shoes,hair): body.data.materials.append(m)
SK,SH,SO,HA=0,1,2,3
me=body.data
xgate=0.17*Hh   # torso/hip half-width; arms/fists hang beyond this
def is_shorts(c,f):
    dx=abs(c.x-cx)
    if dx>=xgate: return False                 # exclude arms/fists
    if 0.45<f<0.56: return True                # waistband + hip block
    if 0.30<f<=0.45 and dx>0.045*Hh: return True  # legs down outer thighs (gap = leg split)
    return False
for poly in me.polygons:
    c=mw@poly.center; f=frac(c.z)
    if f<0.075:            poly.material_index=SO   # shoes/feet
    elif is_shorts(c,f):   poly.material_index=SH   # shaped shorts
    elif f>0.925:          poly.material_index=HA   # hair cap on crown
    else:                  poly.material_index=SK
# world/light
w=bpy.data.worlds.new("W"); bpy.context.scene.world=w; w.use_nodes=True
w.node_tree.nodes.get("Background").inputs[1].default_value=0.0
# camera + pivot for 2 views
emp=bpy.data.objects.new("P",None); bpy.context.collection.objects.link(emp)
ctr=Vector(((min(mw@v.co for v in me.vertices)[i] if False else 0) for i in range(3)))
cx=sum((mw@v.co).x for v in me.vertices)/len(me.vertices)
cy=sum((mw@v.co).y for v in me.vertices)/len(me.vertices)
cz=(zmin+zmax)/2
emp.location=(cx,cy,cz); body.parent=emp; body.matrix_parent_inverse=emp.matrix_world.inverted()
cd=bpy.data.cameras.new("C"); cd.type='ORTHO'; cd.ortho_scale=Hh*1.15
...
sc.render.use_freestyle=True; vl=sc.view_layers[0]; vl.use_freestyle=True
fs=vl.freestyle_settings
if not fs.linesets: fs.linesets.new("ls")
ls=fs.linesets[0]; ls.select_silhouette=True; ls.select_border=True; ls.select_crease=True
ls.select_material_boundary=True; ls.linestyle.color=(0,0,0); ls.linestyle.thickness=2.2
...
for i,yaw in enumerate([0,30]):
    ...
    sc.render.filepath=f"{R}/mascot_cel_{i}.png"; bpy.ops.render.render(write_still=True)
