...
# ---- SHORTEN the shorts: slim the lower-shorts region into thigh (raise the hem) ----
z0=zmin+0.295*Hh; z1=zmin+0.445*Hh
reg=[p for p in vs if z0<p.z<z1]
L=[p for p in reg if p.x<cx]; Rr=[p for p in reg if p.x>=cx]
...
for v in bmh.verts:
    if z0<v.co.z<z1:
        t=(v.co.z-z0)/(z1-z0)          # 0 at bottom (continuous w/ thigh) -> 1 at hem
        f=0.42*(t*t)                    # smooth ease-in, no step at bottom
        ccx,ccy=(lcx,lcy) if v.co.x<cx else (rcx,rcy)
        v.co.x=ccx+(v.co.x-ccx)*(1-f); v.co.y=ccy+(v.co.y-ccy)*(1-f)
...
# ---- body: skin + shoes ----
shoes=principled("Shoes",(0.028,0.030,0.036),0.5)          # dark charcoal
...
for poly in body.data.polygons:
    ...
    elif 0.44<f<0.575:   poly.material_index=2   # shorts above the new (raised) hem
...
sc.render.image_settings.file_format='PNG'; sc.render.filepath=f"{R}/smooth_hero4.png"
