...
# ---- SCULPT PASS: round the blobby fists + carve back muscle ----
def gauss(d,sig): return math.exp(-(d*d)/(2*sig*sig))
bms=bmesh.new(); bms.from_mesh(body.data)
axhalf=max(abs(v.co.x-cx) for v in bms.verts)
# fists: delete the pointy hand blob, record the wrist so we can cap a clean fist
W0=0.72*axhalf   # forearm->fist reshaping starts here
for s in (-1,1):
    wrist=[v for v in bms.verts if 0.70*axhalf < s*(v.co.x-cx) < 0.80*axhalf]
    hb=[v for v in bms.verts if s*(v.co.x-cx) > W0]
    if not wrist or not hb: continue
    acy=sum(v.co.y for v in wrist)/len(wrist); acz=sum(v.co.z for v in wrist)/len(wrist)
    wr_r=sum(math.hypot(v.co.y-acy,v.co.z-acz) for v in wrist)/len(wrist)  # wrist radius
    R=wr_r*1.45                                                            # fist radius
    for v in hb:
        ry=v.co.y-acy; rz=v.co.z-acz; rr=math.hypot(ry,rz) or 1e-6
        u=min(1.0,max(0.0,(s*(v.co.x-cx)-W0)/(axhalf-W0)))                 # 0 forearm ->1 tip
        prof=math.sin(min(1.0,u*1.15)*math.pi*0.5)                        # fatten toward fist, round tip
        targ=wr_r + (R-wr_r)*prof
        v.co.y=acy+ry/rr*targ; v.co.z=acz+rz/rr*targ
    # round the very tip inward in x so it caps instead of a point
    tipx=cx+s*axhalf
    for v in hb:
        u=min(1.0,max(0.0,(s*(v.co.x-cx)-0.90*axhalf)/(axhalf-0.90*axhalf)))
        v.co.x-=s*(tipx-v.co.x)*(-0.0)  # (keep length; profile above already rounds the sides)
forearm=[v for v in bms.verts if abs(v.co.x-cx)>0.55*axhalf]   # declump the twisted forearm + fist
for _ in range(6):
    bmesh.ops.smooth_vert(bms,verts=forearm,factor=0.5,use_axis_x=True,use_axis_y=True,use_axis_z=True)
# back muscle: displace back torso verts (+Y = backward) into spine groove/erectors/lats/traps
for v in bms.verts:
    if v.co.y<=cy: continue
    fz=(v.co.z-zmin)/Hh; dx=abs(v.co.x-cx)
    if fz<0.58 or fz>0.85 or dx>0.16*Hh: continue
    disp = -gauss(dx,0.028*Hh)*0.030*Hh                       # spine groove (indent)
    disp += gauss(dx-0.055*Hh,0.022*Hh)*0.022*Hh              # erector ridge beside spine
    if 0.60<fz<0.74:                                          # lats: spread, wider up (V-taper)
        w=(fz-0.60)/0.14
        disp += gauss(dx-(0.06+0.06*w)*Hh,0.038*Hh)*0.034*Hh
    if 0.76<fz<0.85:                                          # traps: upper center
        disp += gauss(dx,0.06*Hh)*0.028*Hh*((fz-0.76)/0.09)
    v.co.y += disp
bms.to_mesh(body.data); bms.free()
vs=[v.co for v in body.data.vertices]
...
for i,yaw in enumerate([0,45,90,180]):
    piv.rotation_euler=(0,0,math.radians(yaw)); sc.render.filepath=f"{R}/yfinal_{i}.png"
