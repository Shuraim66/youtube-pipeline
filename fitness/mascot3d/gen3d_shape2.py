...
IMG=f"{OUT}/ref/ref2_3d_white.png"
...
print("RESULT stats:", res[2] if isinstance(res,(list,tuple)) and len(res)>2 else "n/a", flush=True)
saved=[]
def grab(x):
    if isinstance(x,str) and os.path.exists(x) and x.lower().endswith(('.glb','.obj','.ply')):
        d=f"{OUT}/mascot2_shape{os.path.splitext(x)[1]}"; shutil.copy(x,d); saved.append(d)
