...
# rest bbox for material assignment (materials are per-face, pose-independent)
rvs=[mw@v.co for v in body.data.vertices]
rzmin=min(p.z for p in rvs); rzmax=max(p.z for p in rvs); rHh=rzmax-rzmin
rcx=sum(p.x for p in rvs)/len(rvs)
def rfrac(z): return (z-rzmin)/rHh
xgate=0.16*rHh
...
def is_shorts(c):
    dx=abs(c.x-rcx); f=rfrac(c.z)
    if dx>=xgate: return False
    if 0.40<f<0.535: return True
    if 0.33<f<=0.40 and dx>0.04*rHh: return True
    return False
for poly in body.data.polygons:
    c=mw@poly.center; f=rfrac(c.z)
...
# ---- POSE: clear anim, bring arms down ----
arm.animation_data_clear(); arm.data.pose_position='POSE'
def setrot(bn,ex,ey,ez):
    pb=arm.pose.bones.get(bn)
    if pb: pb.rotation_mode='XYZ'; pb.rotation_euler=(math.radians(ex),math.radians(ey),math.radians(ez))
setrot('mixamorig:LeftArm', 0,0,-72); setrot('mixamorig:RightArm',0,0,72)
setrot('mixamorig:LeftForeArm',0,0,-12); setrot('mixamorig:RightForeArm',0,0,12)
bpy.context.view_layer.update()

# ---- evaluate POSED mesh for framing + eyes ----
dg=bpy.context.evaluated_depsgraph_get()
ev=body.evaluated_get(dg); em=ev.to_mesh()
pvs=[mw@v.co for v in em.vertices]
zmin=min(p.z for p in pvs); zmax=max(p.z for p in pvs); Hh=zmax-zmin
xs=[p.x for p in pvs]; xmin=min(xs); xmax=max(xs)
cx=(xmin+xmax)/2; cz=(zmin+zmax)/2; cy=sum(p.y for p in pvs)/len(pvs)
head=[p for p in pvs if (p.z-zmin)/Hh>0.90]
hcx=sum(p.x for p in head)/len(head); hcz=sum(p.z for p in head)/len(head); front_y=min(p.y for p in head)
ev.to_mesh_clear()

# ---- eyes (world placed on posed head; head is stationary) ----
...
def add_eye(sx):
    bpy.ops.mesh.primitive_uv_sphere_add(radius=1.0, location=(hcx+sx, front_y-0.004*Hh, hcz+0.010*Hh))
...
# ---- camera framed to POSED bbox (portrait) + light + freestyle ----
...
sc.render.film_transparent=True; sc.render.resolution_x=680; sc.render.resolution_y=1000
sc.render.image_settings.file_format='PNG'; sc.render.filepath=f"{R}/posed_test.png"
bpy.ops.render.render(write_still=True); print("Hh",round(Hh,3),"DONE")
