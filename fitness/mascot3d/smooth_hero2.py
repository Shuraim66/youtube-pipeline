...
bpy.ops.object.transform_apply(location=True,rotation=True,scale=True)  # world==local
...
# ---- head region stats then delete lumpy head ----
CUT=zmin+0.86*Hh
...
htop=max(p.z for p in hv); hbot=CUT; hht=htop-hbot; hcz=(hbot+htop)/2
bm=bmesh.new(); bm.from_mesh(body.data)
dead=[v for v in bm.verts if v.co.z>CUT]
bmesh.ops.delete(bm,geom=dead,context='VERTS'); bm.to_mesh(body.data); bm.free()
# ---- clean ovoid head ----
skin=principled("Skin",(0.33,0.115,0.040),0.42,sss=0.07)   # deep copper
HYS=0.62
bpy.ops.mesh.primitive_uv_sphere_add(segments=48,ring_count=32,radius=1.0,location=(hcx,hcy,hcz-0.045*Hh))
head=bpy.context.active_object; head.name="Head"
head.scale=(hw*0.62, hd*HYS, hht*0.70)
head.data.materials.append(skin); bpy.ops.object.shade_smooth()
# ---- body materials ----
shoes=principled("Shoes",(0.03,0.20,0.19),0.6); shorts=principled("Shorts",(0.04,0.28,0.26),0.62)
body.data.materials.clear()
for m in (skin,shoes,skin,shorts): body.data.materials.append(m)
xg=0.16*Hh
def is_shorts(c):
    dx=abs(c.x-cx); f=frac(c.z)
    if dx>=xg: return False
...
# ---- eyes + logo on the ovoid (known geometry) ----
hf = hcy - hd*HYS        # ovoid front surface y (matches head y-scale)
white=principled("EyeW",(0.95,0.95,0.95),0.35); dark=principled("Logo",(0.06,0.06,0.07),0.4)
EYEZ=hcz-0.055*Hh; LOGOZ=hcz-0.005*Hh
def add_eye(sx):
    bpy.ops.mesh.primitive_uv_sphere_add(radius=1.0,location=(hcx+sx,hf-0.004*Hh,EYEZ))
    e=bpy.context.active_object; e.scale=(hw*0.12,hd*0.04,hw*0.16)
    e.rotation_euler=(0,0,math.radians(-7 if sx<0 else 7)); e.data.materials.append(white); bpy.ops.object.shade_smooth()
add_eye(-hw*0.18); add_eye(hw*0.18)
def ball(x,r):
    bpy.ops.mesh.primitive_uv_sphere_add(radius=r,location=(hcx+x,hf-0.006*Hh,LOGOZ))
    o=bpy.context.active_object; o.scale=(1,0.5,1); o.data.materials.append(dark); bpy.ops.object.shade_smooth()
ball(-hw*0.14,hw*0.05); ball(hw*0.14,hw*0.05)
bpy.ops.mesh.primitive_cylinder_add(radius=hw*0.025,depth=hw*0.30,location=(hcx,hf-0.006*Hh,LOGOZ))
bar=bpy.context.active_object; bar.rotation_euler=(0,math.radians(90),0); bar.scale=(1,0.5,1); bar.data.materials.append(dark)
# ---- world + backdrop + lights + cam (same as before) ----
...
sc.render.image_settings.file_format='PNG'; sc.render.filepath=f"{R}/smooth_hero2.png"
