...
FRONT=f"{OUT}/ref/flat1_white.png"; BACK=f"{OUT}/ref/back1_white.png"
print("connecting...", flush=True)
c=Client("tencent/Hunyuan3D-2")
print("calling /shape_generation multiview (front+back) ...", flush=True)
res=c.predict(caption=None, image=handle_file(FRONT),
    mv_image_front=handle_file(FRONT), mv_image_back=handle_file(BACK),
...
def grab(x):
    if isinstance(x,str) and os.path.exists(x) and x.lower().endswith(('.glb','.obj','.ply')):
        d=f"{OUT}/mascot_mv{os.path.splitext(x)[1]}"; shutil.copy(x,d); saved.append(d)
