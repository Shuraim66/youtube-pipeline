"""Aurelis business-story short — Blockbuster vs Netflix. HOOK-FIRST narrated (energetic VO + AI stills).
DENSE word-matched visuals: 15 phrase-matched Flux stills, one per phrase, ~2.5s cuts (no reuse).
Frame-0 bold hook + bright GRADE_OPEN first frame. Output: prototypes/short_blockbuster.mp4"""
...
IMG="/tmp/claude-1000/-home-alpha-products-youtube-pipeline/d26cef8b-33f8-460d-a296-387d6471b220/scratchpad/nokia/img"
...
GRADE_OPEN=("eq=saturation=1.0:contrast=1.14:brightness=0.05,curves=all='0/0.02 0.25/0.30 0.75/0.86 1/1.0',"
       "colortemperature=temperature=6000:mix=0.3,vignette=PI/10,noise=alls=3:allf=t+u")

SHORTS=[
 {"id":"nokia","speed":1.05,
  "hook":["Half the world's phones.","Gone in 5 years."],
  "punch":"They\nlaughed.","punch_beat":3,
  "cta":["Pride kills giants.","Follow for more"],
  "beats":[
   ("In 2007, Nokia sold half the phones on the planet. Five years later, they sold the entire company for scraps.",
    [f"img:{IMG}/nk_real_phone.jpg",f"img:{IMG}/nk_crowd_phones.jpg",f"img:{IMG}/nk_factory.jpg"]),
   ("Nokia was untouchable. Indestructible phones, billions in profit, the biggest phone maker alive.",
    [f"img:{IMG}/nk_boardroom.jpg",f"img:{IMG}/nk_glass_hq.jpg"]),
   ("Then Apple released the iPhone. A phone with no keyboard, just glass.",
    [f"img:{IMG}/nk_smartphone_stage.jpg",f"img:{IMG}/nk_touchscreen.jpg"]),
   ("Nokia laughed. They said people would never give up real buttons.",
    [f"img:{IMG}/nk_boardroom.jpg",f"img:{IMG}/nk_phone_hand.jpg"]),
   ("But the world wanted the glass screen. Nokia's sales collapsed.",
    [f"img:{IMG}/nk_touchscreen.jpg",f"img:{IMG}/nk_decline_chart.jpg"]),
   ("In 2013, they sold their phone business off. Soon it was worth almost nothing.",
    [f"img:{IMG}/nk_handshake.jpg",f"img:{IMG}/nk_empty_office.jpg"]),
   ("Their CEO said: we didn't do anything wrong. But somehow, we lost.",
    [f"img:{IMG}/nk_ceo_podium.jpg",f"img:{IMG}/nk_old_phones_pile.jpg",f"img:{IMG}/nk_sign_down.jpg"]),
 ]},
]
def dl(url,dst):
...
for S in SHORTS:
    ...
    for i,(terms,a,b) in enumerate(seg):
        ...
        for k,term in enumerate(terms):
            ...
            if term.startswith("img:"):
                imgp=term[4:]; g=GRADE_OPEN if (i==0 and k==0) else GRADE
    ...
    fc=(f"[0:v]ass={D}/subs.ass:fontsdir=/home/alpha/products/openshorts/fonts[b];"
        ...
        f"[1:a]apad,atrim=0:{total:.2f},asetpts=PTS-STARTPTS[vo];[2:a]atrim=0:{total:.2f},afade=t=in:st=0:d=1,afade=t=out:st={total-1.5:.2f}:d=1.5,volume=0.13[mus];"
...
print("BLOCKBUSTER DONE",flush=True)
