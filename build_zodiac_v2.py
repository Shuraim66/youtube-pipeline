...
IMG=ROOT+"/flux_images/zodiac"  # durable FLUX stills (pre-generated on GPU) + case card
REAL=IMG
os.makedirs(IMG,exist_ok=True)
VOICE_K="am_michael"; LANG_K="a"   # Night File narrator: American male, local Kokoro
EMBLEM=ROOT+"/assets/emblem_nf_watermark.png"
import time,random
...
def render_casefile(path,case,fields,stamp="UNSOLVED"):
    ...
    img.save(path)
CASES={
 "zo_file":("The Zodiac Killer",[("Active","1968-1969"),("Location","Northern California"),("Victims","5 confirmed"),("Status","Unidentified")]),
}
SFX=ROOT+"/assets/sfx"
...
SHORTS=[
 {"id":"zodiac","speed":0.97,
  "hook":["A killer who taunted","the police in code.","Never caught."],
  "punch":"Still\nunidentified.","punch_beat":5,
  "cta":["Who was he?","Follow for more"],
  "beats":[
   ("In the late 1960s, a killer terrorized Northern California, and he wanted everyone to know it.",
    ["vid:lonely dark road night fog",f"img:{IMG}/zo_night.jpg"]),
   ("He murdered at least five people, then mailed taunting letters to the newspapers.",
    [f"img:{IMG}/zo_letter.jpg","vid:old newspaper printing press"]),
   ("Here is what investigators knew.",
    [f"card:{IMG}/zo_file.jpg"]),
   ("The letters came with ciphers, coded messages he dared the public to solve.",
    [f"img:{IMG}/zo_cipher.jpg","vid:cryptic handwriting close up dark"]),
   ("One of his codes went unsolved for over fifty years. He signed each note with a crossed-circle symbol.",
    [f"img:{IMG}/zo_cipher.jpg","vid:dark desk lamp old paper"]),
   ("He was never caught. To this day, no one knows who the Zodiac really was.",
    [f"img:{IMG}/zo_shadow.jpg","vid:foggy dark street figure silhouette"]),
 ]},
]
...
for S in SHORTS:
    ...
    fc=(f"[0:v]ass={D}/subs.ass:fontsdir=/home/alpha/products/openshorts/fonts[b];"
        ...
        f"[b][hk]overlay=0:0[v1];[v1][pn]overlay=0:0[v2];[v2][ct]overlay=0:0[vv];[vv][em]overlay=W-w-40:44[v];"
