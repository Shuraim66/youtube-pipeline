...
SCR="/tmp/claude-1000/-home-alpha-products-youtube-pipeline/d26cef8b-33f8-460d-a296-387d6471b220/scratchpad/leadmasks"
...
IMGPROMPTS={
 "lm_masks":"two crude homemade rectangular lead eye masks lying on dark grass, eerie, unsettling, dim light",
 "lm_hill":"a lonely grassy hill under a heavy overcast sky at dusk, ominous, isolated, dread",
 "lm_notebook":"an old handwritten notebook with cryptic notes open on a dark surface, single dim light, unsettling",
}
def render_casefile(path,case,fields,stamp="UNSOLVED"):
    ...
    cf=ImageFont.truetype(SERIF,52); d.text((90,260),case,font=cf,fill=(150,150,155))
    y=430; lf=ImageFont.truetype(SERIFB,46); vf=ImageFont.truetype(SERIF,54)
    for k,v in fields:
        d.text((90,y),k.upper(),font=lf,fill=RED); d.text((90,y+56),v,font=vf,fill=IVORY)
    ...
    img.save(path)
CASES={
 "lm_file":("The Lead Masks Case",[("Date","Aug 1966"),("Location","Vintem Hill, Brazil"),("Victims","2 men"),("Status","Unsolved")]),
}
SFX=ROOT+"/assets/sfx"
...
SHORTS=[
 {"id":"leadmasks","speed":0.97,
  "hook":["Two men found dead.","On a hill.","Wearing lead masks."],
  "punch":"No cause\nof death.","punch_beat":4,
  "cta":["What were they","waiting for?"],
  "beats":[
   ("In 1966, two men were found dead on a hill in Brazil, lying side by side, wearing masks made of lead.",
    ["vid:dark grassy hill overcast sky",f"img:{IMG}/lm_masks.jpg"]),
   ("They were electronics technicians. They told their families they were buying materials, then climbed this hill and never came down.",
    [f"img:{IMG}/lm_hill.jpg","vid:lonely hill fog dusk"]),
   ("Here is what investigators found.",
    [f"card:{IMG}/lm_file.jpg"]),
   ("Beside them was a notebook with strange instructions: be at the site at four thirty, take capsules, protect the metals, wait for the signal.",
    [f"img:{IMG}/lm_notebook.jpg","vid:old handwriting notebook close up"]),
   ("There was no clear cause of death, no sign of a struggle, and no explanation for the lead masks.",
    [f"img:{IMG}/lm_masks.jpg","vid:dark ominous storm clouds"]),
   ("Decades later, the Lead Masks Case is still completely unsolved.",
    ["vid:foggy hill dark","vid:dark clouds moving time lapse"]),
 ]},
]
