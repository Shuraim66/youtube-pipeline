...
IMG=ROOT+"/flux_images/enron"    # durable FLUX stills (pre-generated on GPU) + cards
REAL=IMG
...
def render_card(path,big,label,sub=""):
    ...
    img.save(path)
CARDS={
 "c_peak":("$70B","market value at its peak","named most innovative 6 years running"),
 "c_loss":("20,000","jobs and pensions wiped out","when the fraud unraveled"),
}
SFX=ROOT+"/assets/sfx"
...
SHORTS=[
 {"id":"enron","speed":1.03,
  "hook":["A $70 billion giant.","Gone in weeks."],
  "punch":"Numbers\ncan lie.","punch_beat":5,
  "cta":["Trust the audit.","Follow for more"],
  "beats":[
   ("This was a seventy billion dollar company. Weeks later it was bankrupt, and every dollar of it had been a lie.",
    ["vid:corporate skyscraper city dusk",f"img:{IMG}/en_tower.jpg"]),
   ("For six years running, it was named the most innovative company in America.",
    [f"img:{IMG}/en_trader.jpg","vid:stock trading floor screens"]),
   ("But the profits were an illusion.",
    [f"card:{IMG}/c_peak.jpg"]),
   ("Enron was hiding billions in losses using accounting tricks, and its own auditors let it slide.",
    [f"img:{IMG}/en_shred.jpg","vid:shredding documents paper"]),
   ("When the fraud unraveled, the stock crashed from ninety dollars to almost nothing in weeks.",
    [f"card:{IMG}/c_loss.jpg"]),
   ("It became the biggest bankruptcy in history at the time. The lesson: numbers can lie.",
    ["vid:stock market crash red screen",f"img:{IMG}/en_empty.jpg"]),
 ]},
]
